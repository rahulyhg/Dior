  ...
  */
  ...
  //重新更新 HASH
    ...
    public function mkDelivery($orderIds, $consignee, $corpId, $splitting_product = array()) {
        ...
        {
            $orders    = $this->format_mkDelivery($orders, $items, $splitting_product);
        }
        else
        {
            foreach ($items as $item)
     ...
     * @return Number
     */
    function get_item_product_num($order_id, $item_id, $product_id)
    {
    ...
    public function format_mkDelivery($orders, $items, $splitting_product)
    {
        ...
        $item_product_data      = array();//普通商品与赠品货号相同
        ...
        foreach ($items as $item)
        {
            ...
            if(!empty($splitting_product) && count($orders) == 1)
            {
                ...
                {
                    #[记录]普通商品与赠品货号相同
                    $item_product_data[$split_product_id][$split_item_type]    = $item;
        ...
        #[第一种]订单中有多个相同货号的普通商品  ExBOY
        if(!empty($splitting_product) && !empty($repeat_product_ids))
        {
            $temp_items     = array();
            foreach ($items as $key => $val) 
            {
                $dly_num    = $this->get_item_product_num($val['order_id'], $val['item_id'], $val['product_id']);
                
                $temp_items[$val['item_id']][$val['product_id']]    = intval($val['nums']) - $dly_num;//剩余未发货数量
            }
            ...
            foreach ($orders as $order_id => $order) 
            {
                foreach ($order['items'] as $key => $item_row) 
                {
                    ...
                    if($item_row['item_type'] == 'product' && !empty($repeat_product_ids[$split_product_id]))
                    {
                        ...
                        {
                            #分配发货数量
                            $product_num    = $temp_items[$split_item_id][$split_product_id];//剩余未发货数量
                            $split_num      = $splitting_product[$item_row['item_type']][$split_product_id];//拆分的数量
                            
                            $item_row['nums']    = ($product_num >= $split_num) ? $split_num : $product_num;//允许发货的数量
                            
                            $orders[$order_id]['items'][$split_item_id]['nums']     = $item_row['nums'];
                            ...
                            #剩余可拆分的数量
                            $splitting_product['product'][$split_product_id]    = ($product_num >= $split_num) ? 0 : ($split_num - $product_num);
                        }
                        
                        #[防止]有货号相同的[赠品]存在
                        if(!empty($item_product_data[$split_product_id][$item_row['item_type']]))
                        {
                            $item_product_data[$split_product_id][$item_row['item_type']]['flag']    = 'true';
                        }
                    }
                }
            }
        ...
        #[第二种]订单中有赠品货号与普通商品货号相同 ExBOY
        if(!empty($item_product_data))
        {
            foreach ($item_product_data as $product_id => $item)
            {
                if(!empty($item['product']) && !empty($item['gift']))
                {
                    foreach ($item as $key => $val)
                    {
                        #[防止]多个相同普通商品_已重新计算数量_排除掉
                        if($val['flag'] == 'true')
                        {
                            continue;
                        }
                        ...
                        $product_num    = intval($val['nums']) - $dly_num;//剩余未发货数量
                        $split_num      = $splitting_product['product'][$val['product_id']];//拆分的数量
                        
                        $val['nums']    = ($product_num >= $split_num) ? $split_num : $product_num;//允许发货的数量
                        
                        $orders[$val['order_id']]['items'][$val['item_id']] = $val;//订单明细有则覆盖&&无则增加
                        
                        #没有可拆分的数量
                        if($val['nums'] == 0)
                        {
                            unset($orders[$val['order_id']]['items'][$val['item_id']]);
                        }
                        
                        #剩余可拆分的数量
                        $splitting_product['product'][$val['product_id']]    = ($product_num >= $split_num) ? 0 : ($split_num - $product_num);
                    }
                }
            }
        }
        
        #[第三种]多个订单合并审核时_并且其中包含已"部分拆分"的订单 ExBOY
        if(count($orders) > 1)
        {
            foreach($orders as $order_id => $order)
            {
