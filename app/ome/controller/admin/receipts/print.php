...
class ome_ctl_admin_receipts_print extends desktop_controller {
    ...
    function index() {
        ...
        if($_GET['status'] == 0){
            ...
            $params['actions']['orderbycreatetime'] = array('label'=>$label,'href'=>'index.php?app=ome&ctl=admin_receipts_print&act=orderbycreatetime&p[0]='.$sku.'&p[1]='.$order_val.'&p[2]='.$op_id);
        }
    ...
    function processDeliveryId() {
        ...
        if ($isSelectAll == '_ALL_') {
            ...
            if ($ids) {
                ...
                if (count($logi) > 1)
                    $this->headerErrorMsgDisply("当前系统不支持同时打印二种不同快递类型的单据，请重新选择后再试。");
                if (count($branch) > 1)
                    $this->headerErrorMsgDisply("当前系统不支持同时打印二个仓库的单据，请重新选择后再试。");
                return $dly_ids;
            }
            exit("无数据");
        }else {
    ...
    public function orderbycreatetime($sku,$order_val,$op_id){
        $this->begin('index.php?app=ome&ctl=admin_receipts_print&act=index&status=0&sku='.$sku);
