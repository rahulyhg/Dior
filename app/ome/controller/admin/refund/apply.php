	...
	//kernel::single('omemagento_service_order')->update_status('500000481','refund_failed');exit();
		...
		foreach($arrPayments as $ks=>$vs){
			foreach($vs as $k=>$v){
                ...
                if($v['ship_status']=="0"&&$v['process_status']=="splited"&&$v['shop_type']!="cardshop"){
					echo "订单:".$v['order_bn']."无法退款,已同步给仓库,请及时拦截";exit();
				}
