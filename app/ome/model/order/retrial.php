...
class ome_mdl_order_retrial extends dbeav_model
{
        ...
        foreach ($item_list as $key => $obj_item)
        {
            foreach ($obj_item as $key_j => $items)
            {
                foreach ($items['order_items'] as $key_k => $item)
                {
                    if($show_del)
                    {
                        $product_list['ids'][]  = $item['product_id'];
                        $product_list['nums'][$item['product_id']]  = intval($item['nums']);
                    }
                    else
                    {
    ...
    public function get_create_order_product($order_objects)
    {
        ...
        foreach ($order_objects as $key => $obj_item)
        {
            ...
            {
                $product_list['ids'][]  = $item['product_id'];
                $product_list['nums'][$item['product_id']]  = intval($item['quantity']);
            }
        }
