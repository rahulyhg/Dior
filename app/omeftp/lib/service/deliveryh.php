 ...
 *
 */
class omeftp_service_deliveryh{
    ...
    public function delivery($delivery_id,$sync=false){
		...
		$file_params['file'] = ROOT_DIR.'/ftp/Testing/fuben/'.$file_name.'.dat';
		...
		if($flag){
			...
			if(strpos($file_params['file'],'.csv')){
				$md5_file_local = ROOT_DIR.'/ftp/Testing/fuben/'.basename($file_params['file'],'.csv').'.bal';
			}else{
				$md5_file_local = ROOT_DIR.'/ftp/Testing/fuben/'.basename($file_params['file'],'.dat').'.bal';
			}
	...
	public function get_ax_h($delivery){
		...
		$ax_h[] = $custom_mark[0]['op_content']; //Sales Description
        ...
        switch ($delivery['type']) {
            ...
            case 'normal':  // 普通发货单
                ...
                if (!$delivery['order']) {
                    ...
                    $delivery['order'] = $orderModel->dump(array('order_id'=>$deliOrder['order_id']),'order_bn,cost_payment,shop_id,welcomecard,pmt_order,createtime,invoice_name,cost_tax,invoice_area,invoice_addr,invoice_zip,invoice_contact,is_tax,tax_company,cost_freight,is_delivery,mark_text,custom_mark,sync,ship_area,order_id,self_delivery,createway,pay_bn,pmt_cost_shipping,is_w_card,message1,message2,message3,message4,message5,message6,discount,total_amount');
                }
