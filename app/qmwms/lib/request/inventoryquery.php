...
class qmwms_request_inventoryquery{
    ...
    public function inventery_query(){
        ...
        $offset = 0; $limit = 50;
        do{
            $sql = sprintf("select op.bn,op.store from sdb_ome_products op left join sdb_ome_goods og on op.goods_id = og.goods_id where 1=1 order BY op.product_id ASC limit %u,%u ",$offset,$limit);//bn in('G002471','G002472') 条件仅调试使用
