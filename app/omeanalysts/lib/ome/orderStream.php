<?php
class omeanalysts_ome_orderStream extends eccommon_analysis_abstract implements eccommon_analysis_interface{
    ...
    public function finder(){
        ...
        $params =  array(
            'model' => 'omeanalysts_mdl_ome_orderStream',
            'params' => array(
                'actions'=>array(
                     array(
                          'class' => 'export',
                         'label' => '生成报表',
                         'href'=>'index.php?app=omeanalysts&ctl=ome_analysis&act=order_stream&action=export',
                         'target'=>'{width:600,height:300,title:\'生成报表\'}'
                     ),
                ),
                'title'=>app::get('omeanalysts')->_('订单流水<script>if($$(".finder-list").getElement("tbody").get("html") == "\n" || $$(".finder-list").getElement("tbody").get("html") == "" ){$$(".export").set("href","javascript:;").set("onclick", "alert(\"没有可以生成的数据\")");}else{$$(".export").set("href",\'index.php?app=omeanalysts&ctl=ome_analysis&act=order_stream&action=export\');}</script>'),
