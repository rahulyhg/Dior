...
kernel::single('base_shell_webproxy')->exec_command("install tgkpi");
kernel::single('base_shell_webproxy')->exec_command("install taoexlib");
...
foreach ($otmpl as $key=>$value) {
    $is_exist = is_exist_print_otmpl($key);
...
function is_exist_print_otmpl($type) 
{
...
function save_print_otmpl($data) 
{
